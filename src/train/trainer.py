...
class SimCLR(object):
    ...
    def train_MIL(self, root, epoch_count, args):
        ...
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=75, gamma=0.5, last_epoch=-1)
        optimal_ac = 0
        auc_list = []
        auc_list_val = []
        optimal_ac_val = 0
        ...
        selected_auc = auc_list_val_optimal_test[-1] if len(auc_list_val_optimal_test) > 0 else 0.0
        self.experiment.log_metric('Test_MIL_auc_sel_by_val', selected_auc, epoch=epoch_count)
